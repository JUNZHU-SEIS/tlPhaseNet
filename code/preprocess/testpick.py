...
def show(data, itp, its, fpath, fname):
	color1="#E65A63"; color2="#379958"; color3="#505599"
	onset = itp
	fig, ax = plt.subplots(3,1, sharex=True)
	ax[0].plot(data[:,0], color=color1, lw=0.8, label='E')
	ax[0].axvline(x=itp, c='gray', ls='--')
	ax[0].axvline(x=its, c='gray', ls='--')
	rect = plt.Rectangle((onset, -1000000), its-itp, 2000000, color="lightgray")
	ax[0].add_patch(rect)
	ax[1].plot(data[:,1], color=color2, lw=0.8, label='N')
	ax[1].axvline(x=itp, c='gray', ls='--')
	ax[1].axvline(x=its, c='gray', ls='--')
	rect = plt.Rectangle((onset, -1000000), its-itp, 2000000, color="lightgray")
	ax[1].add_patch(rect)
	ax[2].plot(data[:,2], color=color3, lw=0.8, label='Z')
	ax[2].axvline(x=itp, c='gray', ls='--')
	ax[2].axvline(x=its, c='gray', ls='--')
	rect = plt.Rectangle((onset, -1000000), its-itp, 2000000, color="lightgray")
	ax[2].add_patch(rect)
	plt.suptitle('_'.join((fpath, fname)))
#	plt.savefig('samplewaveform.png', dpi=1000)
	plt.show()
	return
...
#					continue
				...
				print(itp, its, fpath, fname)
				show(npz_data, itp, its, fpath, fname)
